/**
 * Seed script — inserts the 20 Callousing Challenges into Firestore.
 * Run once: npx ts-node --project tsconfig.json scripts/seed-challenges.ts
 * Re-running is safe (uses set + merge:true with deterministic IDs).
 */
...
const challenges: ChallengeData[] = [
  // Difficulty 1 — Free tier (exactly 5)
  {
...
async function seed() {
  ...
  for (const { id, ...data } of challenges) {
    batch.set(db.collection("challenges").doc(id), data, { merge: true });
  }
  await batch.commit();
  console.log("Done.");
  process.exit(0);
